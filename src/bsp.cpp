...
static void bsp_tasks_create() {
    ...
    // appTask stack MORA biti v SRAM (ne PSRAM).
    // RAZLOG: appTask kliče LittleFS (vehicle_recog_init → load_baseline) in NVS
    //   (config_save). esp_flash_read() zahteva cache-disable → PSRAM ni dostopen.
    //   IDF assert: esp_task_stack_is_sane_cache_disabled() → panic če stack v PSRAM.
    // SRAM bilanca je OK: s_sd_buf→PSRAM + mDNS odstranjen = +16KB SRAM.
    //   18340 B po web_ui_begin zadostuje za AsyncTCP cbuf + SD DMA + rezervo.
